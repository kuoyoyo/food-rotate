...
final class HistoryStorage {
    ...
    /// 這樣「錯誤被吞掉」就不可能再重新長出來。
    ///
    /// ## 這裡沒有整合測試，是決定不是遺漏（S6 P3-1）
    ///
    /// 我們考慮過「讓 `context.save()` 真的失敗」的整合測試（唯讀容器／磁碟滿／schema 衝突），
    /// 結論是**不做**：
    ///
    /// - 要驗的其實是「錯誤有沒有被吞掉」，而那件事已經收斂在這一個方法裡；
    ///   分支只有兩條，`HistoryStorageTests` 用 `recordSave(error:)` 兩邊都走過了。
    /// - 「SwiftData 在磁碟滿的時候會不會 throw」是在測 Apple，不是在測我們。
    /// - 那條路唯一的後果是「提示沒跳出來」，而提示的狀態機有測試背書。
    ///
    /// 所以**如果哪天要加測試，加在狀態機那一層，不要去製造真實的儲存失敗。**
